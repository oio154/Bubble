...
function Circle(x,y,dx,dy,radius){
  ...
  this.draw = function(){
    ...
    c.strokeStyle = `rgba(0,0,0,${(this.radius/20)})`;
...
for(var z=0;z<400;z++){
 circles.push(new Circle(innerWidth*Math.random()/2,innerHeight*Math.random(),20*(Math.random() - .5),20*(Math.random() - .5),20*Math.random()));
}
